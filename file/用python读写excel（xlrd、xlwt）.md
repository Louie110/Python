...
```
> 若干坑：
    - 在excel中输入的时候很可能会顺手在一些名字后面加上几个空格或是tab键，
    这样看起来没什么差别，但是程序处理的时候这就是两个完全
    不同的串了。解决方法是给每个获取的字符串都加上strip（）处理一下。效果良好。
    - 还是字符串的匹配，在判断某个单元格中的字符串（中文）是否等于我所给出的的时候
    发现无法匹配，并且各种unicode也不太奏效，采用了一个比较变通的方式：直接从excel
    中获取想要的值再进行比较，效果是不错就是通用性不太好。

 ## 二、写excel表
 写excel表要用到xlwt模块，大致使用流程如下：
1. 导入模块
`import xlwt`
